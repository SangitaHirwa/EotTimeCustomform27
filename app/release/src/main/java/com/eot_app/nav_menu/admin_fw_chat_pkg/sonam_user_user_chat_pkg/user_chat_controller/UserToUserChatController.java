...
public class UserToUserChatController {
    ...
    String[] chatScreenStateArray = new String[]{"", ""};
    private boolean state;
    private UserToUserChatActivity userToUserChatActivity;
    HashMap<String, UserOflineOnlineModel> user_Status_Model_List = new LinkedHashMap<>();
    ...
    public void createChatNotifications(MsgModel model) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            if (model.getContent() != null && !model.getContent().equals("")) {
                notification = nNotificationUtils.getNotificationManager(userChatModel.getFnm(),
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            if (userChatModel != null) {
                if (model.getMsg() != null && model.getMsg().equals("📷 Photo")) {
                    ...
                    nb = nNotificationUtils.getAndroidChannelNotification(0,
                            userChatModel.getFnm(), FILE_SHARED, resultIntent).setWhen(System.currentTimeMillis()).setShowWhen(true);
                    nNotificationUtils.getManager().notify(notIdEmergency, nb.build());
                } else {
                    nb = nNotificationUtils.getAndroidChannelNotification(0,
                            userChatModel.getFnm(),
                            model.getMsg(), resultIntent).setWhen(System.currentTimeMillis()).setShowWhen(true);
                    nNotificationUtils.getManager().notify(notIdEmergency, nb.build());
                }
            }
        } else if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
