...
public class EqipAttchAdpter extends RecyclerView.Adapter<EqipAttchAdpter.MyViewHolder> {
    ...
    public void onBindViewHolder(EqipAttchAdpter.MyViewHolder holder, int position) {
        String ext = "";
        try {
            ext = jobdata.get(position).getImage_name().substring((jobdata.get(position).getImage_name().lastIndexOf(".")) + 1).toLowerCase();
        } catch (Exception e) {
            e.printStackTrace();
        }
        try {
            if (!ext.isEmpty()) {
                if (ext.equals("jpg") || ext.equals("jpeg") || ext.equals("png")) {
                    Glide.with(context)
                            .load(App_preference.getSharedprefInstance().getBaseURL() + jobdata.get(position).getAttachThumnailFileName())
