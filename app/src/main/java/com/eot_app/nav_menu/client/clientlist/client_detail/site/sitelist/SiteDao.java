...
public interface SiteDao {
    ...
    // to get the enabled or active sites  only
    @Query("select *from Site_model where cltId= :cltId and isactive=1 order by LOWER(snm) ASC")
    List<Site_model> getActiveSitesFromCltId(int cltId);
    ...
    //TODO is active param
//    @Query("select *from Site_model where cltId= :cltId order by LOWER(snm) ASC")
    // to get the enabled or active sites  only
    @Query("select *from Site_model where cltId= :cltId and isactive=1 order by LOWER(snm) ASC")
    List<Site_model> getSitesByCltId(String cltId);

    @Query("select * from Site_model where cltId = :cltId and isactive=1")
    Site_model getSiteByClientId(String cltId);

    // to get the enabled or active sites  only
    @Query("select * from Site_model where cltId = :cltId and isactive=1")
    Site_model getActiveSiteByClientId(String cltId);
