...
public class Job_Filter_PC implements Job_Filter_Pi {
    ...
    public void setStatusList() {
        ...
        //TODO
        List<JobStatusModelNew> jobStatusListDynamic = AppDataBase.getInMemoryDatabase(EotApp.getAppinstance()).jobStatusModelNew().getFwStatusList();

        // for removing the multistatus job status from filter
        JobStatusModelNew jobStatusModelNew = null;
        for (JobStatusModelNew jobStatus:jobStatusListDynamic) {
            if(jobStatus.getStatus_no().equals("11")){
                jobStatusModelNew=jobStatus;
            }
        }
        if(jobStatusModelNew!=null){
            jobStatusListDynamic.remove(jobStatusModelNew);
        }
       ...
       /* final ArrayList<JobStatusModel> jobStatusList = new ArrayList<>();
        ...
        // this is actual on hold
        jobStatusList.add(new JobStatusModel(AppConstant.New_On_Hold, LanguageController.getInstance().getMobileMsgByKey(AppConstant.new_on_hold)));
       ...
       */
        job_filter_view.setstatus(jobStatusListDynamic);
    }
    ...
    public void setJobPrityList() {
        ...
        jobPrioties.add(new JobPrioty(AppConstant.Medium_id, AppConstant.medium_caps));
