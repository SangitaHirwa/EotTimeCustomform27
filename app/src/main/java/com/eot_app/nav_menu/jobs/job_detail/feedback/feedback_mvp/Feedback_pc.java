...
public class Feedback_pc implements Feedback_pi {
    ...
    public void UploadSignToServer(File mypath, String DesNotes, String setSmilie, String mParam2) {
        ...
        if (mypath != null) {
            ...
            RequestBody requestFile = RequestBody.create(mypath,MediaType.parse(mimeType));
            // MultipartBody.Part is used to send also the actual file name
            body = MultipartBody.Part.createFormData("sign", mypath.getName(), requestFile);
        }
        RequestBody description = RequestBody.create(DesNotes,MultipartBody.FORM);

        RequestBody rating = RequestBody.create(String.valueOf(setSmilie),MultipartBody.FORM);

        RequestBody jobId = RequestBody.create(mParam2,MultipartBody.FORM);

        RequestBody userId = RequestBody.create(App_preference.getSharedprefInstance().getLoginRes().getUsrId(),MultipartBody.FORM);

        //RequestBody token = RequestBody.create(okhttp3.tokenstr);
