...
public class JobDetailActivity extends AppCompatActivity implements
        ...
        ViewPager.OnPageChangeListener, NotifyForAddJob,
        NotificationObserver, Job_Detail_Activity_View,
        ResultCallback<LocationSettingsResult> {//, NotifyForInvoiceGenr
...
//                                LanguageController.getInstance().getMobileMsgByKey(AppConstant.invoice_can_not_generate_msg), null,
                ...
                case ID_JOB_EQUIPMENT:
                    ...
                    intent1.putExtra("siteid", dataJob.getSiteId());
    ...
    protected void onDestroy() {
        ...
        //  EotApp.getAppinstance().setNotifyForInvoiceGenr(null);
...
//                        BottomNavigationMenuView bottomNavigationMenuView = (BottomNavigationMenuView) navigation.getChildAt(0);
            ...
            if (serverList.isEnable.equals("1"))
                switch (serverList.getMenuField()) {
                    case "set_jobDetailMenuOdrNo":
                        menu_items_ilist.add(new MenuItemsModel(ID_JOB_DETAIL, LanguageController.getInstance().getMobileMsgByKey(AppConstant.job_details), R.drawable.ic_detail));
                        break;
    ...
    public void setList(ArrayList<CustomFormList_Res> customFormLists) {
        if (this.customFormLists != null) {
            this.customFormLists.clear();
        }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (item.getItemId() == android.R.id.home) {
            ...
            {
                String[] fwList2=dataJob.getKpr().split(",");
                dialogJobCardDocuments.setFwList(fwList2);
            }
            dialogJobCardDocuments.setdetail_activity_pi(detail_activity_pi);
            if(templateList!=null && !templateList.isEmpty()){
    ...
    public void openFormForEvent(String eventId, String completeFor, String jobType) {
        ...
        if (App_preference.getSharedprefInstance().getLoginRes().getCustomFormEnable().equals("1")) {
            ...
            try {
                if (AppUtility.isInternetConnected()) {
                    if (customFormLists != null && customFormLists.size() > 0) {
                        ...
                        if (i == 0) {
                            ...
                            HyperLog.i(TAG, "openFormForEvent(M) : Custom FORM is not there ............");
                        }
                    } else {
                        if (detailFragment != null) {
                            detailFragment.onFormSuccess(completeFor, jobType);
                ...
                ex.printStackTrace();
                HyperLog.i(TAG, "openFormForEvent(M) :" + ex);
                if (detailFragment != null) {
                    detailFragment.onFormSuccess(completeFor, jobType);
                } else {
                    HyperLog.i(TAG, "openFormForEvent(M) : detailFragment NULL......");
    ...
    public void onNotifyListner(String key, final String msg) {
        if (key.equals("complition")) {
            ...
            if (dataJob.getJobId().equals(key)) {
                runOnUiThread(() -> AppUtility.alertDialog(JobDetailActivity.this, LanguageController.getInstance().getMobileMsgByKey(AppConstant.dialog_alert), msg, LanguageController.getInstance().getMobileMsgByKey(AppConstant.ok), "", () -> {
                    finish();
                    return null;
                }));
            }
...
//                 ,""
    ...
    public void onSignatureUpload(String signaturePath, String customerName, String msg) {
        ...
        AppUtility.alertDialog(JobDetailActivity.this, "", msg, LanguageController.getInstance().getMobileMsgByKey(AppConstant.ok), "", () -> null);
    ...
    public void setInvoiceTmpList(ArrayList<InvoiceEmaliTemplate> templateList) {
        AppUtility.progressBarDissMiss();
        Log.e("TemplateData::", "setList:" + new Gson().toJson(templateList));
    ...
    protected void onHideKeyboard() {
        if (navigation.getVisibility() == View.GONE) {
            new Handler().postDelayed(() -> navigation.setVisibility(View.VISIBLE), 10);
        }
    }
    ...
    protected void onRestart() {
        Log.e("Activity", "onRestartCalled");
        super.onRestart();
        AppUtility.progressBarDissMiss();
    }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        inflater.inflate(R.menu.job_card_menu, menu);
        return true;
    }

    @Override
    public boolean onPrepareOptionsMenu(Menu menu) {
        menu.findItem(R.id.miJobCard).setVisible
                (App_preference.getSharedprefInstance().getLoginRes().getCompPermission().get(0).getIsJobCardEnableMobile()
                        != null && App_preference.getSharedprefInstance().getLoginRes().getCompPermission().get(0).
                        getIsJobCardEnableMobile().equals("0"));

        menu.findItem(R.id.miJobCard).setTitle(LanguageController.getInstance().getMobileMsgByKey(AppConstant.job_card));
    ...
    public void openCustomSignatureDialog(String statusId) {
        if (detail_activity_pi != null && dataJob != null) {
            ...
            if (TextUtils.isEmpty(dataJob.getSignature())) {
                ...
                clear_txt.setOnClickListener(v -> mSignature.clear());
    ...
    @Override
    public void updateJobId(String tempId, String jobId) {
        Log.e("OnCreateDetail", "Add Job Notify Called");
        dataJob = AppDataBase.getInMemoryDatabase(EotApp.getAppinstance()).jobModel().getJobsById(jobId);
        Log.e("OnCreateDetail", "dataJob" + new Gson().toJson(dataJob));
        // for updating the job id every where and setting the current selected page in viewpager
        isJobUpdated = true;
    ...
    class MypagerAdapter extends FragmentStatePagerAdapter {
        ...
        public Fragment getItem(int position) {
            switch (position) {
                case 0:
                    if (detailFragment == null || isJobUpdated) {
                        detailFragment = DetailFragment.newInstance("No", dataJob.getJobId());
                    }
                    return detailFragment;
                case 1:
                    if (documentsFragment == null || isJobUpdated) {
